...
public class PlayerLayerHandler {
    ...
    public static void setupRenders(EntityRenderersEvent.AddLayers event) {
        ...
        if (playerRendererDefault != null) {
            playerRendererDefault.addLayer(new ForceFieldLayerRender<>(playerRendererDefault, event.getEntityModels(), false));
        }
        if (playerRendererSlim != null) {
            playerRendererSlim.addLayer(new ForceFieldLayerRender<>(playerRendererSlim, event.getEntityModels(), true));
        }
    }
}
